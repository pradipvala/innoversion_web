...
@push('url_title') Franchise  @endpush
      ...
      <div class="col-md-12">
        <div class="card-box">
            <div class="row form-horizontal">
              <form class="form-horizontal" id="franchise-form" role="form" method="POST" action="{{ route('admin.franchise.update') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="franchise_id" id="franchise_id" value="{{$franchise->id}}">
                <div class="form-group">
                    <div class="col-sm-3">
                      <label><b>Business name</b> :- </label>
                          <input type="text" name="business_name" id="business_name"  value="{{ isset($franchise->business_name) &&!empty($franchise->business_name)?$franchise->business_name:NULL;}}" class="form-control" />
                    </div>

                    <div class="col-sm-3">
                      <label><b>Franchise name</b> :- </label>
                          <input type="text" name="franchise_name" id="franchise_name"  class="form-control" value="{{ isset($franchise->franchise_name) &&!empty($franchise->franchise_name)?$franchise->franchise_name:NULL;}}"/>
                    </div>

                    <div class="col-sm-3">
                      <label><b>Gst no.</b> :- </label>
                          <input type="text" name="gst_no" id="gst_no" class="form-control"  value="{{ isset($franchise->gst_no) &&!empty($franchise->gst_no)?$franchise->gst_no:NULL;}}"/>
                    </div>

                    <div class="col-sm-3">
                      <label><b>Contact No.</b> :- </label>
                          <input type="tel" name="contact_no" id="contact_no" class="form-control" value="{{ isset($franchise->contact_no) &&!empty($franchise->contact_no)?$franchise->contact_no:NULL;}}" />
                    </div>
                
                    <div class="col-sm-3">
                      <label><b>Email Address</b> :- </label>
                      <input type="email" name="email" id="email" class="form-control"  value="{{ isset($franchise->email) &&!empty($franchise->email)?$franchise->email:NULL;}}"/>
                    </div>
                    ...
                    <div class="form-group col-sm-3">
                      ...
                      <select class="form-select form-control select2" aria-label="Default select example" name="c_name" id="city_id" value="">
                          ...
                          <option value="{{ $city1->id }}" {{ $data->city_id == $city1->id?'selected':'' }}>{{ $city1->c_name }}</option>
                          @endforeach
                      </select>
                    </div> 
                
                    <div class="col-sm-3">
                      <labe><b>Contact Person Name</b> :- </label>
                      <input type="text" name="contact_person" id="contact_person" class="form-control"  value="{{ isset($franchise->contact_person) &&!empty($franchise->contact_person)?$franchise->contact_person:NULL;}}"/>
                    </div>

                    <div class="col-sm-3">
                      <labe><b>Aadhaar Card No.</b> :- </label>
                      <input type="text" name="aadhaar_no" id="aadhaar_no" class="form-control"  value="{{ isset($franchise->aadhaar_no) &&!empty($franchise->aadhaar_no)?$franchise->aadhaar_no:NULL;}}"/>
                    </div>

                    <div class="col-sm-3">
                      <label><b>Pan Card no.</b> :- </label>
                      <input type="text" name="pan_no" id="pan_no" class="form-control"  value="{{ isset($franchise->pan_no) &&!empty($franchise->pan_no)?$franchise->pan_no:NULL;}}" />
                    </div>

                    <div class="col-sm-3">
                      <label><b>Upload Pan Card</b> :- </label>
                        @if (isset($franchise->pan_img))
                            <img src="{{ \Storage::url($franchise->pan_img) }}" height="80px"
                                width="80px"><br /><br />
                                <input type="file" class="filestyle form-control"  name="pan_img" id="pan_img" 
                  ...
                  <div class="col-sm-3">
                    <label><b>Upload Aadhaar Card</b> :- </label>
                      @if (isset($franchise->aadhaar_img))
                          <img src="{{ \Storage::url($franchise->aadhaar_img) }}" height="80px"
                              width="80px"><br /><br />
                              <input type="file" class="filestyle form-control"  name="aadhaar_img" id="aadhaar_img"
                                      accept="image/jpeg, image/jpeg, image/png" >
                          @if (count($errors) > 0)
                              @foreach ($errors->all() as $error)
                                  <span style="color:red">{{ $error }}</span>
                  ...
                  <div class="col-sm-3">
                    <label><b>Upload GST Certificate</b> :- </label>
                      @if (isset($franchise->gst_certificate))
                          <img src="{{ \Storage::url($franchise->gst_certificate) }}" height="80px"
                              width="80px"><br /><br />
                              <input type="file" class="filestyle form-control"  name="gst_certificate" id="gst_certificate" 
                                      accept="image/jpeg, image/jpeg, image/png">
                          @if (count($errors) > 0)
                              @foreach ($errors->all() as $error)
                                  <span style="color:red">{{ $error }}</span>
                  ...
                  <div class="approval_status_div col-sm-3" id="approval_status_div" >
                      <?php 
                      
                        $check1 = $check2 = '';
                        
                        if(isset($franchise->approval_status)&&!empty($franchise->approval_status)){

                          if($franchise->approval_status =='Approved'){
                            $check1 ='checked'; 
                          }
                              
                          else if($franchise->approval_status =='Rejected'){
                            $check2 ='checked'; 
                          }
                        }
                      ?>
