...
@push('url_title') Web Support List @endpush
@section('title')
@push('content')
    ...
    <div class="col-sm-12">
        <div class="card-box table-responsive">
            <div class="m-t-0 m-b-10 row">
              <div class="col-sm-4">
                <h4 class="m-t-0 header-title"><b>Web Support List</b></h4>
              </div>
               <!-- @can('web-support-create') -->
                <div class="col-sm-8 text-right">
                    <a class="btn btn-primary" href="{{ route('admin.create.websupport') }}" role="button"> <span>
                    <i class="fa fa-plus"></i></span> Create Web Support</a>
                </div><br><br>
                 <!-- @endcan  -->
            </div>
                 ...
                 <div class="row">
                     <div class="col-sm-12">
                         <div class="table-responsive">
                             <table id="web-support-datatable" class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                      <th>Sr No.</th>
                                      <th>Category</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if($('#web-support-datatable').length > 0) {

            $('#web-support-datatable').dataTable({
                    ...
                    "ajax": {
                          "url": "{{ route('admin.websupport.listing') }}",
                          "type": "GET",
                          "dataType": "json",
                          "data": {
                              _token: "{{csrf_token()}}"
        ...
        $(document).on('click','#status',function(){
          ...
          var id = $(this).val();
            ...
            $.ajax({
              type: "POST",
              url: "{{ route('admin.change.status.websupport') }}",
    ...
    function delete_func(object){
        ...
        {
            $.ajax({
                "url": "{!! route('admin.websupport.delete') !!}",
                "dataType": "json",
                "type": "POST",
                "data":{
                    id: id,
                ...
                success: function (data){
                    if(data.status == 200){
                         toastr.success('Web support data deleted successfully', 'Success');
                            $('#web-support-datatable').DataTable().ajax.reload();                  
                    }else{
                        toastr.error('Failed to delete web support data', 'Error');
                    }
                }
            });
        }else{
